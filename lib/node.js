...
if(process.env.NODE_ENV === 'production' && INSTANCE_NAME === "production")
{
...
function Node ()
{
	this.info = {
		...
		contact: (process.env.CONTACT_DETAILS || "cortex network"),
		coinbase: null,
		node: null,
		net: null,
		protocol: null,
		api: null,
		port: (process.env.LISTENING_PORT || 40404),
		os: os.platform(),
		os_v: os.release(),
...
Node.prototype.startWeb3Connection = function()
{
	...
	web3 = new Web3();
	web3.setProvider(new web3.providers.HttpProvider('http://' + (process.env.RPC_HOST || '127.0.0.1') + ':' + (process.env.RPC_PORT || '8545')));
...
Node.prototype.startSocketConnection = function()
{
	if( !this._socket )
	{
		...
		socket = new Socket( process.env.WS_SERVER || 'ws://127.0.0.1:3000' );
	...
	if (this._web3 && (lastFetchAgo >= UPDATE_INTERVAL || forced === true))
	{
		...
		function (err, results)
		{
			...
			if(results.peers !== null)
			{
				...
				self.stats.hashrate = results.diff;//results.hashrate;
